	...
	plugin_subscriptions{}
{
    ...
    // naodai:send motor command 
	motor_sub = nh.subscribe("/mavros/actuator_control", 100, &MavRos::mavlink_sub_motor, this,
		ros::TransportHints()
			.unreliable().maxDatagramSize(1024)
			.reliable());

    // naodai: send light command
    light_sub = nh.subscribe("/mavros/light_control", 100, &MavRos::mavlink_sub_light, this,
		ros::TransportHints()
			.unreliable().maxDatagramSize(1024)
			.reliable());

	// naodai: send gripper command
	gripper_sub = nh.subscribe("/mavros/gripper_control", 100, &MavRos::mavlink_sub_gripper, this,
		ros::TransportHints()
			.unreliable().maxDatagramSize(1024)
 			.reliable());
    
	// naodai: send gripper command 
	// t265_rePower_sub = nh.subscribe("/mavros/t265_rePower", 100, &MavRos::mavlink_sub_t265_rePower, this,
	    // ros::TransportHints()
			// .unreliable().maxDatagramSize(1024)
 			// .reliable());
...
// naodai: motor
void MavRos::mavlink_sub_motor(const mavros_msgs::ActuatorControl::ConstPtr &req)
{
	mavlink::common::msg::SET_ACTUATOR_CONTROL_TARGET act{};
	
    act.time_usec = req->header.stamp.toNSec() / 1000;
    act.group_mlx = req->group_mix;
    // act.target_system = &mav_uas->get_tgt_system();
    // act.target_component = &mav_uas->get_tgt_component();
    std::copy(req->controls.begin(), req->controls.end(), act.controls.begin());    // std::array = boost::array
    UAS_FCU(&mav_uas)->send_message_ignore_drop(act);
}


// naodai: light
void MavRos::mavlink_sub_light(const mavros_msgs::Param::ConstPtr &req)
{
	mavlink::common::msg::PARAM_VALUE act{};
    // up light
	act.param_index = req->param_index;
	// down light
	act.param_count = req->param_count;
	UAS_FCU(&mav_uas)->send_message_ignore_drop(act);
}

// naodai: gripper
void MavRos::mavlink_sub_gripper(const mavros_msgs::Thrust::ConstPtr &req)
{
	mavlink::common::msg::PING act{};
	act.seq = req->thrust;	
	UAS_FCU(&mav_uas)->send_message_ignore_drop(act);

}

// naodai: t265 opened or closed
// void MavRos::mavlink_sub_t265_rePower(const mavros_msgs::ParamValue::ConstPtr &req)
// {
	...
	// uint16 param_index
	// uint16 param_count
    // mavlink::common::msg::COMMAND_INT act{};
	// act.param1 = req -> integer;
	// UAS_FCU(&mav_uas)->send_message_ignore_drop(act);
// }
